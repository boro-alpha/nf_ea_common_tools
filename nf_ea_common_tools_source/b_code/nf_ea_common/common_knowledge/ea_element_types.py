...
from nf_ea_common_tools_source.b_code.nf_ea_common.common_knowledge.ea_object_types import EaObjectTypes


@unique
class EaElementTypes(
        EaObjectTypes):
    ACTION = auto()
    ACTION_PIN = auto()
    ACTIVITY = auto()
    ...
    INTERFACE = auto()
    INTERRUPTIBLE_ACTIVITY_REGION = auto()
    ISSUE = auto()
    LABEL = auto()
    LOOP_NODE = auto()
    MERGE_NODE = auto()
    ...
    def get_type_from_name(
            name: str):
        for ea_object_type, ea_object_type_name in type_name_mapping.items():
            if ea_object_type_name == name:
                return \
                    ea_object_type
