...
def reorder_ea_attributes(
    ...
    for com_object in NfUuidsToIDualObjectsMappings.get_i_dual_elements():
...
def __reorder_attributes_if_required(
        com_object,
        classifier_id_order: dict):
    if isinstance(com_object, IDualElement):
...
def __reorder_attributes(
        ...
        classifier_id_order: dict):
    if i_dual_element.attributes.count < 2:
        return
    ...
    for index in range(i_dual_element.attributes.count):
        new_positions = \
            __add_new_position(
    ...
    new_order = \
        {k: v for k, v in sorted(new_positions.items(), key=lambda item: item[1])}

    index = \
        0

    for i_dual_attribute in new_order.keys():
        index = \
            __reorder_attribute(
...
def __add_new_position(
        ...
        -> dict:
    attribute_at_index = \
        i_dual_element.attributes.get_at(
            index)

    classifier_id = \
        attribute_at_index.classifier_id

    if classifier_id in classifier_id_order:
        position = \
            classifier_id_order[classifier_id]

    else:
        position = \
            index + max(classifier_id_order.values())

    new_positions[attribute_at_index] = \
        position
