...
from ea_interop_service_source.b_code.factories.i_dual_connector_factory import create_i_dual_connector
...
from nf_ea_common_tools_source.b_code.services.session.processes.nf_ea_com_loaders.ea_stereotypes_load_helper import get_ea_stereotype_ex
...
def load_ea_connectors(
    ...
    for index, ea_connector_row in tqdm(ea_connectors.iterrows(), total=ea_connectors.shape[0]):
        ea_connector = \
            __load_ea_connector(
                nf_uuid=ea_connector_row[nf_uuids_column_name],
                ea_connector_name=ea_connector_row[ea_connector_name_column_name],
                ea_connector_type=ea_connector_row[ea_connector_type_column_name],
                child_nf_uuid=ea_connector_row[ea_object_place1_column_name],
                parent_nf_uuid=ea_connector_row[ea_object_place2_column_name],
                note=ea_connector_row[ea_connector_note_column_name],
                stereotype_usage_with_names=stereotype_usage_with_names)

        if isinstance(ea_connector, INullConnector):
            ea_connectors.at[index, ea_guid_column_name] = \
                DEFAULT_NULL_VALUE

        elif isinstance(ea_connector, IDualConnector):
            ea_connectors.at[index, ea_guid_column_name] = \
...
def __load_ea_connector(
    ...
    if not isinstance(child, IDualElement):
        return \
            INullConnector()
    ...
    if not isinstance(parent, IDualElement):
        return \
            INullConnector()
