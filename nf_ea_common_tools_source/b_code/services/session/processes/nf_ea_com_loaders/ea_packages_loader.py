...
from ea_interop_service_source.b_code.factories.i_dual_package_factory import create_i_dual_package
...
def load_ea_packages(
    ...
    for index, ea_package_row in tqdm(ea_packages.iterrows(), total=ea_packages.shape[0]):
        ea_package = \
            __load_ea_package(
                ...
                package_view_type=ea_package_row[package_view_type_column_name])

        ea_packages.at[index, ea_guid_column_name] = \
...
def __load_ea_package(
    ...
    if not isinstance(ea_package_parent, IDualPackage):
        raise \
            TypeError

    if package_view_type == DEFAULT_NULL_VALUE:
        package_view_type = \
