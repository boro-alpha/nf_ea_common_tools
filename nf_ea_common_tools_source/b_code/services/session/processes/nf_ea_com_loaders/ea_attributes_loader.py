...
from ea_interop_service_source.b_code.factories.i_dual_attribute_factory import create_i_dual_attribute
...
def load_ea_attributes(
    ...
    for index, ea_attribute_row in tqdm(ea_attributes.iterrows(), total=ea_attributes.shape[0]):
        ea_attribute = \
            __load_ea_attribute(
                ea_attribute_name=ea_attribute_row[name_column_name],
                ea_attribute_type_nf_uuid=ea_attribute_row[classifying_ea_classifier_column_name],
                attributed_object_nf_uuid=ea_attribute_row[containing_ea_classifier_column_name],
                ea_attribute_type_name=ea_attribute_row[ea_attribute_type_name_column_name],
                ea_attribute_initial_value=ea_attribute_row[ea_attribute_initial_value_column_name])

        ea_attributes.at[index, ea_guid_column_name] = \
...
def __load_ea_attribute(
    ...
    if not isinstance(attributed_object, IDualElement):
        raise \
            TypeError
    ...
    if ea_attribute_type_nf_uuid in registered_i_dual_element_nf_uuids:
        attribute_type = \
            NfUuidsToIDualObjectsMappings.get_i_dual_element(
                nf_uuid=ea_attribute_type_nf_uuid)

        if not isinstance(attribute_type, IDualElement):
            raise \
                TypeError
    ...
    if ea_attribute_initial_value == DEFAULT_NULL_VALUE:
