...
def load_ea_proxy_connectors(
    ...
    for index, ea_classifier_row in tqdm(ea_proxy_connectors.iterrows(), total=ea_proxy_connectors.shape[0]):
        ea_classifier = \
            __load_ea_proxy_connector(
                nf_uuid=ea_classifier_row[nf_uuid_column_name],
                ea_package_parent_nf_uuid=ea_classifier_row[parent_column_name],
                ea_proxy_connector_name=ea_classifier_row[name_column_name],
                ea_proxy_connector_type=ea_classifier_row[type_column_name],
                ea_classifier_nf_uuid=ea_classifier_row[classifier_column_name])

        ea_proxy_connectors.at[index, ea_guid_column_name] = \
...
def __load_ea_proxy_connector(
    ...
    ea_classifier = \
        NfUuidsToIDualObjectsMappings.get_i_dual_connector(
            nf_uuid=ea_classifier_nf_uuid)

    if not isinstance(ea_classifier, IDualConnector):
        raise \
            TypeError
