...
from ea_interop_service_source.b_code.factories.i_dual_element_factory import create_i_dual_element
...
from nf_ea_common_tools_source.b_code.services.session.processes.nf_ea_com_loaders.ea_stereotypes_load_helper import \
...
def load_ea_classifiers(
    ...
    for index, ea_classifier_row in tqdm(ea_classifiers.iterrows(), total=ea_classifiers.shape[0]):
        ea_classifier = \
            __load_ea_classifier(
                nf_uuid=ea_classifier_row[nf_uuid_column_name],
                ea_package_parent_nf_uuid=ea_classifier_row[parent_column_name],
                ea_classifier_name=ea_classifier_row[name_column_name],
                ea_classifier_type=ea_classifier_row[type_column_name],
                ea_classifier_nf_uuid=ea_classifier_row[classifier_column_name],
                ea_classifier_notes=ea_classifier_row[notes_column_name],
                stereotype_usage_with_names=stereotype_usage_with_names)

        ea_classifiers.at[index, ea_guid_column_name] = \
...
def __load_ea_classifier(
        ...
        -> IDualElement:
    ea_package_parent = \
        NfUuidsToIDualObjectsMappings.get_i_dual_package(
            nf_uuid=ea_package_parent_nf_uuid)

    if ea_classifier_notes == DEFAULT_NULL_VALUE:
        ea_classifier_notes = \
            ''
    ...
    if ea_classifier_nf_uuid == DEFAULT_NULL_VALUE:
        return \
            ea_classifier
    ...
    if not isinstance(ea_classifier_classifier, IDualElement):
        raise \
            TypeError
