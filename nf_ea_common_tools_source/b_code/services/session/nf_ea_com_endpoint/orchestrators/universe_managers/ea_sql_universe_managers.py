...
from nf_ea_common_tools_source.b_code.services.general.ea.sql.ea_sql_universes import EaSqlUniverses
from nf_ea_common_tools_source.b_code.services.session.nf_ea_com_endpoint.orchestrators.nf_managers import NfManagers


class EaSqlUniverseManagers(
    NfManagers):
    def __init__(
            self):
        NfManagers.__init__(
    ...
    def get_ea_t_table_dataframe(
            ...
            -> DataFrame:
        ea_sql_universe = \
            self.__get_ea_sql_universe(
                ea_repository=ea_repository)

        ea_t_table_dataframe = \
            ea_sql_universe.get_ea_t_table_dataframe(
    ...
    def export_all_registries(
        ...
        for ea_repository, ea_sql_universe in self.ea_sql_universe_dictionary.items():
    ...
    def __get_ea_sql_universe(
            ...
            ea_repository: EaRepositories) \
            -> EaSqlUniverses:
        if ea_repository in self.ea_sql_universe_dictionary:
            return \
                self.ea_sql_universe_dictionary[ea_repository]

        ea_sql_universe = \
            self.__create_ea_sql_universe(
                ea_repository=ea_repository)
