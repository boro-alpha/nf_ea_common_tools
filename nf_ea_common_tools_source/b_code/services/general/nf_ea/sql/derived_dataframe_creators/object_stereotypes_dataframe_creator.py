...
def __filter_to_stereotype_usages(
        ...
        -> DataFrame:
    object_stereotypes_dataframe = \
        t_xref_extended_dataframe.loc[t_xref_extended_dataframe[EaTXrefColumnTypes.T_XREF_NAMES.nf_column_name] == 'Stereotypes']

    object_stereotypes_dataframe = \
        object_stereotypes_dataframe.loc[
            (object_stereotypes_dataframe[EaTXrefColumnTypes.T_XREF_TYPES.nf_column_name] == EaPropertyTypes.CONNECTOR_PROPERTY.type_name) |
            (object_stereotypes_dataframe[EaTXrefColumnTypes.T_XREF_TYPES.nf_column_name] == EaPropertyTypes.ELEMENT_PROPERTY.type_name) |
            (object_stereotypes_dataframe[EaTXrefColumnTypes.T_XREF_TYPES.nf_column_name] == EaPropertyTypes.ATTRIBUTE_PROPERTY.type_name)]

    object_stereotypes_dataframe = \
        object_stereotypes_dataframe.fillna(
            DEFAULT_NULL_VALUE)
...
def __convert_description_to_list_of_guids(
        ...
        -> DataFrame:
    object_stereotypes_dataframe['t_xref_descriptions_as_list'] = \
        object_stereotypes_dataframe[EaTXrefColumnTypes.T_XREF_DESCRIPTIONS.nf_column_name].apply(
            lambda x: x.split(';'))

    object_stereotypes_dataframe['guids_texts'] = \
        object_stereotypes_dataframe['t_xref_descriptions_as_list'].apply(
            lambda text_items:
            [text_item for text_item in text_items if text_item.startswith('GUID')])

    object_stereotypes_dataframe[ExtendedTObjectColumnTypes.LIST_OF_STEREOTYPE_GUIDS.column_name] = \
        object_stereotypes_dataframe['guids_texts'].apply(
            lambda guid_items: __remove_guid_name_from_guid_items(guid_items))
...
def __convert_list_of_guids_to_rows(
    ...
    empty_expanded_list_dataframe = \
        DataFrame(
            {
                column: numpy.repeat(object_stereotypes_dataframe[column].values,
                                     object_stereotypes_dataframe[list_column_name].str.len())
                for column in object_stereotypes_dataframe.columns.drop(list_column_name)
            })

    if object_stereotypes_dataframe.shape[0] == 0:
        return \
            DataFrame(
                columns=
                [
    ...
    expanded_list_array = \
        numpy.concatenate(object_stereotypes_dataframe[list_column_name].values)

    expanded_list_dataframe = \
        empty_expanded_list_dataframe.assign(
            **{list_column_name: expanded_list_array})

    expanded_list_dataframe = \
        expanded_list_dataframe.filter(
            items=
            [
                EaTXrefColumnTypes.T_XREF_CLIENT_EA_GUIDS.nf_column_name,
                'list_of_stereotype_guids',
    ...
    expanded_list_dataframe = \
        expanded_list_dataframe.rename(
            columns=
            {
...
def __add_nf_uuids(
    ...
    object_stereotype_usage_dataframe = \
        __add_collection_type_objects_to_usage_dataframe(
            object_stereotypes_dataframe=object_stereotypes_dataframe,
    ...
    nf_uuids_object_stereotypes_dataframe = \
        concat(
            [
                object_stereotype_usage_dataframe,
                connector_stereotype_usage_dataframe,
    ...
    object_stereotypes_dataframe = \
        left_merge_dataframes(
            master_dataframe=object_stereotypes_dataframe,
            master_dataframe_key_columns=['stereotype_guids'],
            merge_suffixes=['_usage', '_stereotype'],
            foreign_key_dataframe=extended_t_stereotypes_dataframe,
            foreign_key_dataframe_fk_columns=[EaTStereotypesColumnTypes.T_STEREOTYPES_EA_GUIDS.nf_column_name],
            foreign_key_dataframe_other_column_rename_dictionary={
...
def __add_collection_type_objects_to_usage_dataframe(
        ...
        -> DataFrame:
    collection_type_object_stereotypes_dataframe = \
        inner_merge_dataframes(
            master_dataframe=object_stereotypes_dataframe,
            master_dataframe_key_columns=[EaTXrefColumnTypes.T_XREF_CLIENT_EA_GUIDS.nf_column_name],
            merge_suffixes=['_usage', '_collection_type'],
            foreign_key_dataframe=extended_collection_type_dataframe,
            foreign_key_dataframe_fk_columns=[collection_type_ea_guid_nf_column_name],
            foreign_key_dataframe_other_column_rename_dictionary={
