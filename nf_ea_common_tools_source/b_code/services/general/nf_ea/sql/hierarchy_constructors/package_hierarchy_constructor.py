...
def construct_package_hierarchy_dataframe(
    ...
    package_hierarchy_dataframe = \
        package_hierarchy_dataframe.rename(
            columns=
            {
    ...
    package_hierarchy_dataframe['paths'] = \
        package_hierarchy_dataframe['package_names_0']

    level = 0

    while package_hierarchy_dataframe['package_names_' + str(level)].nunique() > 1:
        level = \
            level + 1

        package_hierarchy_dataframe = \
            __add_level_to_hierarchy(
                level=level,
...
def __add_level_to_hierarchy(
    ...
    package_hierarchy_dataframe = \
        left_merge_dataframes(
            ...
            master_dataframe_key_columns=['package_ids_' + str(level)],
            merge_suffixes=['_' + str(level - 1), '_' + str(level)],
            foreign_key_dataframe=t_package_dataframe,
            foreign_key_dataframe_fk_columns=[EaTPackageColumnTypes.T_PACKAGE_IDS.nf_column_name],
            foreign_key_dataframe_other_column_rename_dictionary=
            {
                EaTPackageColumnTypes.T_PACKAGE_EA_GUIDS.nf_column_name: 'package_ea_guids_' + str(level),
                EaTPackageColumnTypes.T_PACKAGE_NAMES.nf_column_name: 'package_names_' + str(level),
                EaTPackageColumnTypes.T_PACKAGE_PARENT_IDS.nf_column_name: 'package_ids_' + str(level + 1)
            })

    package_hierarchy_dataframe = \
        package_hierarchy_dataframe.fillna(
    ...
    package_hierarchy_dataframe['paths'] = \
        package_hierarchy_dataframe.apply(
            lambda row:
            row['paths']
            if row['package_names_' + str(level)] == DEFAULT_NULL_VALUE
            else row['package_names_' + str(level)] + '/' + row['paths'],
            axis=1)
