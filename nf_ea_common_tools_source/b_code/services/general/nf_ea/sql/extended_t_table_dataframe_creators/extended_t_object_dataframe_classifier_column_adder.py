...
def add_classifier_column_to_extended_t_object_dataframe(
    ...
    extended_t_object_dataframe = \
        __extend_extended_t_object_dataframe_with_classifier_data(
...
def __extend_extended_t_object_dataframe_with_classifier_data(
    ...
    extended_t_object_non_proxy_connectors_dataframe_with_classifier_column = \
        __extend_filtered_extended_t_object_with_classifier_column(
            extended_t_object_dataframe=extended_t_object_dataframe,
...
def __extend_filtered_extended_t_object_with_classifier_column(
    ...
    if is_proxy_connector:
        filtered_extended_t_objects = \
            extended_t_object_dataframe.loc[extended_t_object_dataframe[object_type_column_name] == EaElementTypes.PROXY_CONNECTOR.type_name]
    else:
        filtered_extended_t_objects = \
            extended_t_object_dataframe.loc[extended_t_object_dataframe[object_type_column_name] != EaElementTypes.PROXY_CONNECTOR.type_name]
    ...
    filtered_extended_t_objects = \
        left_merge_dataframes(
            ...
            master_dataframe_key_columns=[EaTObjectColumnTypes.T_OBJECT_CLASSIFIER_GUIDS.nf_column_name],
            merge_suffixes=['_master', '_linked'],
            foreign_key_dataframe=classifier_dataframe,
            foreign_key_dataframe_fk_columns=[classifier_dataframe_ea_guid_column_name],
