...
def extend_with_stereotypes_data(
    ...
    dataframe = \
        left_merge_dataframes(
            ...
            merge_suffixes=['_master', '_stereotype'],
            foreign_key_dataframe=stereotypes_dataframe,
            foreign_key_dataframe_fk_columns=[EaTXrefColumnTypes.T_XREF_CLIENT_EA_GUIDS.nf_column_name],
    ...
    for row in dataframe.loc[dataframe['list_of_stereotype_guids'].isnull(), 'list_of_stereotype_guids'].index:
        dataframe.at[row, 'list_of_stereotype_guids'] = \
            []
...
def __get_stereotypes_dataframe(
        ...
        -> DataFrame:
    stereotypes_dataframe = \
        __get_base_stereotypes_dataframe(
            nf_ea_sql_universe=nf_ea_sql_universe,
            type_filter_text=type_filter_text)
...
def __get_base_stereotypes_dataframe(
    ...
    base_stereotypes_dataframe = \
        t_xref_dataframe.loc[t_xref_dataframe[EaTXrefColumnTypes.T_XREF_NAMES.nf_column_name] == 'Stereotypes']

    base_stereotypes_dataframe = \
        base_stereotypes_dataframe.loc[base_stereotypes_dataframe[EaTXrefColumnTypes.T_XREF_TYPES.nf_column_name] == type_filter_text]

    base_stereotypes_dataframe = \
        base_stereotypes_dataframe.fillna(
            DEFAULT_NULL_VALUE)
...
def __convert_description_to_list_of_guids(
        ...
        -> DataFrame:
    stereotypes_dataframe['t_xref_descriptions_as_list'] = \
        stereotypes_dataframe[EaTXrefColumnTypes.T_XREF_DESCRIPTIONS.nf_column_name].apply(
            lambda x: x.split(';'))

    stereotypes_dataframe['guids_texts'] = \
        stereotypes_dataframe['t_xref_descriptions_as_list'].apply(
            lambda text_items:
            [text_item for text_item in text_items if text_item.startswith('GUID')])

    stereotypes_dataframe['list_of_stereotype_guids'] = \
        stereotypes_dataframe['guids_texts'].apply(
