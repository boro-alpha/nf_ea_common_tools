...
from nf_ea_common_tools_source.b_code.nf_ea_common.common_knowledge.column_types.extended_t.extended_t_object_column_types import \
...
def create_extended_t_xref_dataframe(
    ...
    extended_t_xref_dataframe = \
        __extend_with_connector_data(
            extended_t_xref_dataframe=extended_t_xref_dataframe,
            extended_t_connector_dataframe=extended_t_connector_dataframe)

    extended_t_xref_dataframe['t_xref_ea_human_readable_names'] = \
        '(xref' + \
        '-' + extended_t_xref_dataframe[EaTXrefColumnTypes.T_XREF_TYPES.nf_column_name] + \
        '-' + extended_t_xref_dataframe[EaTXrefColumnTypes.T_XREF_NAMES.nf_column_name] + ') ' + \
        extended_t_xref_dataframe['t_xref_t_object_names']
...
def __extend_with_object_data(
    ...
    extended_t_xref_dataframe = \
        left_merge_dataframes(
            ...
            merge_suffixes=['_xref', '_object'],
            foreign_key_dataframe=extended_t_object_dataframe,
            foreign_key_dataframe_fk_columns=[EaTObjectColumnTypes.T_OBJECT_EA_GUIDS.nf_column_name],
    ...
    extended_t_xref_dataframe = \
        extended_t_xref_dataframe.fillna(
            DEFAULT_NULL_VALUE)
...
def __extend_with_connector_data(
    ...
    extended_t_xref_dataframe = \
        left_merge_dataframes(
            ...
            master_dataframe_key_columns=[EaTXrefColumnTypes.T_XREF_CLIENT_EA_GUIDS.nf_column_name],
            merge_suffixes=['_xref', '_connector'],
            foreign_key_dataframe=extended_t_connector_dataframe,
            foreign_key_dataframe_fk_columns=[EaTConnectorColumnTypes.T_CONNECTOR_EA_GUIDS.nf_column_name],
            foreign_key_dataframe_other_column_rename_dictionary=
            {
                'start_t_object_paths': 't_xref_t_connector_start_paths'
            })

    extended_t_xref_dataframe['t_xref_container_paths'] = \
        extended_t_xref_dataframe.apply(
            lambda row:
            row['t_xref_t_connector_start_paths']
            if row['t_xref_container_paths'] == DEFAULT_NULL_VALUE
            else row['t_xref_container_paths'],
            axis=1)

    extended_t_xref_dataframe = \
        extended_t_xref_dataframe.drop(
            columns=['t_xref_t_connector_start_paths'])
