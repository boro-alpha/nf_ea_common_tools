...
class EaPackageContentsSummaryFactories:
    ...
    def create(
        ...
        ea_package_contents_summary = \
            self.__add_ea_package_contents_counts(
    ...
    def create_ea_package_contents_summary(
        ...
        ea_package_contents_summary = \
            ea_package_contents_summary.filter(
                items=items)
    ...
    def __add_ea_package_contents_counts(
        ...
        ea_package_full_contents_counts = \
            self.__create_contents_counts(
        ...
        ea_package_nearest_contents_counts = \
            self.__create_contents_counts(
        ...
        for column in ea_package_contents_counts:
            columns_to_add_dictionary[column] = \
                column

        del columns_to_add_dictionary[nf_uuids_column_name]

        ea_package_contents_summary = \
            left_merge_dataframes(
                master_dataframe=ea_package_contents_summary,
                master_dataframe_key_columns=[nf_uuids_column_name],
                merge_suffixes=['_summary', '_counts'],
        ...
        ea_package_contents_summary = \
            ea_package_contents_summary.fillna(
                0)
    ...
    def __create_contents_counts(
        ...
        ea_package_contents_counts = \
            crosstab(
                ea_parent_child_packages['parent'],
                ea_parent_child_packages['object_type'])
        ...
        for column in ea_package_contents_counts:
            column_rename_dictionary[column] = \
                'type_' + prefix + '_' + column

        ea_package_contents_counts = \
            ea_package_contents_counts.reset_index()

        ea_package_contents_counts = \
            ea_package_contents_counts.rename(
    ...
    def __add_hash(
            ea_package_contents_summary: DataFrame) \
            -> DataFrame:
        ea_package_contents_summary['immutable_hash'] = \
            ea_package_contents_summary.apply(
                lambda row:
                EaPackageContentsSummaryFactories.__get_composite_ea_guid(
                    row),
                axis=1)
    ...
    def __get_composite_ea_guid(
            ...
            -> str:
        package_ea_guid = \
            row[NfEaComColumnTypes.EXPLICIT_OBJECTS_EA_GUID.column_name]

        package_name = \
            row[NfEaComColumnTypes.EXPLICIT_OBJECTS_EA_OBJECT_NAME.column_name]
        ...
        for column_name in ordered_row_index:
            if column_name.startswith('type_full'):
                type_full_count = \
                    row[column_name]

                if type_full_count > 0:
                    objects.append(
                        column_name + '=' + str(type_full_count))
    ...
    def __reorder(
            ...
            -> DataFrame:
        ea_package_contents_summary = \
            ea_package_contents_summary.sort_values(
                'ea_package_path')
