...
def __rename_nf_ea_diagram(
    ...
    i_dual_diagram = \
        i_dual_repository.get_diagram_by_guid(
            diagram_ea_guid=diagram_guid)

    if isinstance(i_dual_diagram, INullDiagram):
        log_message(
            diagram_guid + "Warning: Diagram not found")

        return

    if not isinstance(i_dual_diagram, IDualDiagram):
        raise \
            TypeError
    ...
    if old_ea_diagram_name == new_ea_diagram_name:
        log_message(
            diagram_guid + "Current name equals clean name for Diagram Name (" + old_ea_diagram_name + ")")
        ...
        log_message(
            diagram_guid + " : Diagram Name changed (" + old_ea_diagram_name + ") to (" + new_ea_diagram_name + ")")
