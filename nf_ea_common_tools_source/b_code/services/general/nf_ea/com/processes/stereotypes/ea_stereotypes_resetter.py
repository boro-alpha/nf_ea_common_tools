...
from nf_ea_common_tools_source.b_code.services.session.processes.nf_ea_com_loaders.ea_repository_getter import \
    get_ea_repository
...
def reset_all_stereotypes_using_manager(
    ...
    log_message(
        message='Resetting all stereotypes in ' + ea_repository_file.base_name)
...
def __reset_all_stereotypes_in_package_and_subpackages(
    ...
    log_message(
        message='Resetting stereotypes in package ' + i_dual_package.name)

    __reset_all_stereotypes_in_package(
        i_dual_package=i_dual_package)

    packages = \
        i_dual_package.packages

    for index in range(0, packages.count):
        subpackage = \
            packages.get_at(
                index=index)
...
def __reset_all_stereotypes_in_package(
        i_dual_package: IDualPackage):
    elements = \
        i_dual_package.elements

    for index in range(0, elements.count):
        element = \
            elements.get_at(
                index=index)
    ...
    for index in range(0, connectors.count):
        connector = \
            connectors.get_at(
                index=index)
...
def __reset_all_stereotypes_for_element(
    ...
    for index in range(0, attributes.count):
        attribute = \
            attributes.get_at(
                index=index)
...
def __reset_stereotype_in_ea_object(
    ...
    if hasattr(ea_object, 'stereotype_ex'):
...
def __try_update_ea_object(
    ...
    try:
        ...
        log_message(
            message='Cannot update ' + ea_object.name + ' because of ' + str(exception.args))
