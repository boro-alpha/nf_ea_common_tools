...
from nf_ea_common_tools_source.b_code.services.general.nf_ea.com.processes.model_stats.common.constants import \
    SOURCE_COLUMN_NAME, TARGET_COLUMN_NAME, LEVEL_COLUMN_NAME, EA_CONNECTOR_ELEMENT_TYPE_NAME_COLUMN_NAME, \
    DEPENDENCY_NAME, EA_OBJECT_NAME_COLUMN_NAME, SUPPLIER_PLACE1_END_CONNECTORS_COLUMN_NAME, \
    CLIENT_PLACE2_END_CONNECTORS_COLUMN_NAME, IMPLICIT_DEPENDENCY_NAME
...
def get_edges_table_from_table_with_unique_paths_column(
        ...
        -> DataFrame:
    paths_list = \
        table_with_unique_paths_column[path_column_name].tolist()

    edges_with_level = \
        __generate_edges_dataframe_with_level(
            paths_list=paths_list)

    edges_with_attributes = \
        __add_attributes_to_edges(
            edges_dataframe=edges_with_level,
            ea_connectors=ea_connectors)

    full_dependencies_edges_dataframe = \
        __add_values_to_implicit_edges(
...
def __generate_edges_dataframe_with_level(
    ...
    for path in paths_list:
        for source_node, target_node in zip(path[:-1], path[1:]):  # original: zip(path[1:], path[:-1])
            edge_level = \
                path.index(
                    target_node)
    ...
    edges_dataframe = \
        DataFrame.from_dict(
            edges_dictionary).drop_duplicates()

    edges_dataframe_max_level = \
        edges_dataframe.groupby(
            [SOURCE_COLUMN_NAME,
             TARGET_COLUMN_NAME], as_index=False)[LEVEL_COLUMN_NAME].max()
...
def __add_attributes_to_edges(
        ...
        ea_connectors: DataFrame) \
        -> DataFrame:
    ea_connectors_filtered = \
        ea_connectors.loc[ea_connectors[EA_CONNECTOR_ELEMENT_TYPE_NAME_COLUMN_NAME] == DEPENDENCY_NAME]
    ...
    edges_dataframe_attributes_added = \
        left_merge_dataframes(
            master_dataframe=edges_dataframe,
            master_dataframe_key_columns=[SOURCE_COLUMN_NAME, TARGET_COLUMN_NAME],  # Previously: [TARGET_COLUMN_NAME, SOURCE_COLUMN_NAME]
            merge_suffixes=['1', '2'],
            foreign_key_dataframe=ea_connectors_filtered,
            foreign_key_dataframe_fk_columns=[SUPPLIER_PLACE1_END_CONNECTORS_COLUMN_NAME,
    ...
    edges_dataframe_with_ea_connectors_attributes = \
