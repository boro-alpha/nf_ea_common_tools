...
from nf_ea_common_tools_source.b_code.services.general.nf_ea.com.processes.model_stats.common.constants import PACKAGE_NAMES_COLUMN_NAME, STEREOTYPE_NAMES_COLUMN_NAME
...
def add_data_columns_to_nodes_base_table(
        ...
        -> DataFrame:
    ea_classifiers_with_package_names = \
        left_merge_dataframes(
            master_dataframe=ea_classifiers,
            master_dataframe_key_columns=[NfEaComColumnTypes.PACKAGEABLE_OBJECTS_PARENT_EA_ELEMENT.column_name],
            merge_suffixes=['1', '2'],
    ...
    nodes_table_with_ea_classifiers_data = \
        left_merge_dataframes(
            master_dataframe=nodes_base_table,
            master_dataframe_key_columns=[NfColumnTypes.NF_UUIDS.column_name],
            merge_suffixes=['1', '2'],
    ...
    ea_stereotype_usage_with_stereotype_names = \
        left_merge_dataframes(
            master_dataframe=ea_stereotype_usage,
            master_dataframe_key_columns=[StandardConnectorTableColumnTypes.STEREOTYPE_NF_UUIDS.column_name],
            merge_suffixes=['1', '2'],
    ...
    nodes_table_with_stereotypes_data = \
        left_merge_dataframes(
            master_dataframe=nodes_table_with_ea_classifiers_data,
            master_dataframe_key_columns=[NfColumnTypes.NF_UUIDS.column_name],
            merge_suffixes=['1', '2'],
            foreign_key_dataframe=ea_stereotype_usage_with_stereotype_names,
            foreign_key_dataframe_fk_columns=[NfEaComColumnTypes.STEREOTYPE_CLIENT_NF_UUIDS.column_name],
            foreign_key_dataframe_other_column_rename_dictionary={
    ...
    nodes_table_with_data_columns = \
        nodes_table_with_stereotypes_data.drop_duplicates(
            ignore_index=True)

    all_column_names_but_stereotype_names = \
        nodes_table_with_data_columns.columns.tolist()

    if nodes_table_with_data_columns.empty:
        nodes_table_with_data_columns = \
            DataFrame(
