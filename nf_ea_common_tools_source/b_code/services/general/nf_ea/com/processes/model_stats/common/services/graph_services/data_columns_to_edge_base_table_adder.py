...
from nf_ea_common_tools_source.b_code.services.general.nf_ea.com.processes.model_stats.common.constants import STEREOTYPE_NAMES_COLUMN_NAME
from nf_ea_common_tools_source.b_code.services.general.nf_ea.com.processes.model_stats.common.services_promote_to_nf_common.dataframe_into_dictionary_of_rows_converter import convert_dataframe_into_dictionary_of_rows
from nf_ea_common_tools_source.b_code.services.general.nf_ea.com.processes.model_stats.common.services_promote_to_nf_common.dictionary_mergers import left_merge_dictionaries_of_rows
...
def add_data_columns_to_edge_base_table(
        ...
        -> DataFrame:
    edge_base_table_dictionary = \
        convert_dataframe_into_dictionary_of_rows(
            dataframe=edge_base_table)

    ea_connectors_dictionary = \
        convert_dataframe_into_dictionary_of_rows(
            dataframe=ea_connectors)

    ea_stereotypes_dictionary = \
        convert_dataframe_into_dictionary_of_rows(
            dataframe=ea_stereotypes)

    ea_stereotype_usage_dictionary = \
        convert_dataframe_into_dictionary_of_rows(
            dataframe=ea_stereotype_usage)

    edges_table_with_ea_connectors_data_dictionary = \
    ...
    ea_stereotype_usage_with_stereotype_names_dictionary = \
        left_merge_dictionaries_of_rows(
    ...
    edges_table_with_data_columns_dictionary = \
        left_merge_dictionaries_of_rows(
    ...
    edges_table_with_data_columns = \
        edges_table_with_data_columns.groupby(
            all_column_names_but_stereotype_names)[STEREOTYPE_NAMES_COLUMN_NAME].apply(','.join).reset_index()
