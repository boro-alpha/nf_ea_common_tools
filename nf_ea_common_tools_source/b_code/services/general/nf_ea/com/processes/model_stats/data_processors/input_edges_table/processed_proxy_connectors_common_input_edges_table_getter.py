...
from nf_common_source.code.services.dataframe_service.dataframe_mergers import outer_merge_dataframes
...
from nf_ea_common_tools_source.b_code.services.general.nf_ea.com.processes.model_stats.common.constants import NF_UUIDS2_COLUMN_NAME
from nf_ea_common_tools_source.b_code.services.general.nf_ea.com.processes.model_stats.common.constants import UUID_COLUMN_NAME
from nf_ea_common_tools_source.b_code.services.general.nf_ea.com.processes.model_stats.common.constants import CLASSIFIER_COLUMN_NAME
from nf_ea_common_tools_source.b_code.services.general.nf_ea.com.processes.model_stats.common.services.dataframe_services.uuid_column_generator import add_uuid_column_to_dataframe
from nf_ea_common_tools_source.b_code.services.general.nf_ea.com.processes.model_stats.data_processors.input_edges_table.input_edges_base_table_human_readable_fields_adder import add_human_readable_fields_to_input_edges_base_table
...
def get_processed_proxy_connectors_common_input_edges_table(
        ...
        -> DataFrame:
    ea_connectors_only_new_broken_down_connectors = \
        __get_new_broken_down_connectors_from_proxy_connectors(
            ea_connectors=ea_connectors,
            ea_classifiers=ea_classifiers)

    ea_connectors_only_new_broken_down_connectors_uuidified = \
        __add_uuids_to_new_connectors_dataframe(
            connectors_dataframe=ea_connectors_only_new_broken_down_connectors)

    ea_connectors_without_proxy_connectors = \
        __remove_proxy_connectors_from_ea_connectors(
            ea_classifiers=ea_classifiers,
            ea_connectors=ea_connectors)

    processed_proxy_connectors_common_input_edges_base_table = \
        concat(
            [ea_connectors_only_new_broken_down_connectors_uuidified,
             ea_connectors_without_proxy_connectors]).reset_index()

    processed_proxy_connectors_common_input_edges_base_table = \
        processed_proxy_connectors_common_input_edges_base_table.filter(
            items=[
    ...
    processed_proxy_connectors_input_edges_human_readable_table = \
...
def __get_new_broken_down_connectors_from_proxy_connectors(
    ...
    temporary_dataframe_with_supplier_place1_end_connector = \
        __get_new_broken_down_connector_half(
            ea_connectors=ea_connectors,
            ea_classifiers=ea_classifiers,
    ...
    new_broken_down_connectors_from_proxy_connectors = \
        concat(
            [temporary_dataframe_with_supplier_place1_end_connector,
             temporary_dataframe_with_client_place2_end_connector]
...
def __add_uuids_to_new_connectors_dataframe(
        ...
        -> DataFrame:
    uuidified_new_connectors_dataframe = \
...
def __get_new_broken_down_connector_half(
        ...
        connector_place: str) \
        -> DataFrame:
    ea_classifiers_filtered_to_proxy_connectors = \
        ea_classifiers.loc[ea_classifiers[
            NfEaComColumnTypes.ELEMENTS_EA_OBJECT_TYPE.column_name].isin(
...
def __add_connector_type_name_to_new_connectors(
        new_connectors_dataframe: DataFrame,connector_place:str) \
        -> DataFrame:

    connection_type_list = \
        new_connectors_dataframe[
            NfEaComColumnTypes.CONNECTORS_ELEMENT_TYPE_NAME.column_name].to_list()

    for connector_type in new_connectors_dataframe[NfEaComColumnTypes.CONNECTORS_ELEMENT_TYPE_NAME.column_name]:
        if connector_type in connection_type_list:
            new_connectors_dataframe[
                NfEaComColumnTypes.CONNECTORS_ELEMENT_TYPE_NAME.column_name] = \
                connector_type + connector_place
...
def __remove_proxy_connectors_from_ea_connectors(
    ...
    temporary_dataframe_no_proxy_connectors_filtered = \
        temporary_dataframe_no_proxy_connectors[
            temporary_dataframe_no_proxy_connectors[
                UUID_COLUMN_NAME + '2'].isnull()].reset_index()

    temporary_ea_connectors_dataframe_columns = \
        {
            UUID_COLUMN_NAME + '1': UUID_COLUMN_NAME,
    ...
    temporary_ea_connectors_dataframe = \
        dataframe_filter_and_rename(
            ...
            filter_and_rename_dictionary=temporary_ea_connectors_dataframe_columns).drop_duplicates()

    clean_ea_connectors_dataframe = \
        temporary_ea_connectors_dataframe[
            temporary_ea_connectors_dataframe[
                NfEaComColumnTypes.ELEMENTS_SUPPLIER_PLACE1_END_CONNECTORS.column_name].notnull()].reset_index(drop=True)
