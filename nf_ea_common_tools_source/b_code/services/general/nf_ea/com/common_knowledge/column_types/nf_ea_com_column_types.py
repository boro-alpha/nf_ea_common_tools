...
class NfEaComColumnTypes(
    ColumnTypes):
    CLASSIFIERS_ALL_COMPONENT_EA_ATTRIBUTES = auto()
    CLASSIFIERS_ALL_COMPONENT_EA_OPERATIONS = auto()
    CLASSIFIERS_CONTAINING_EA_ELEMENT = auto()
    ELEMENTS_EA_OBJECT_TYPE = auto()
    ELEMENTS_SUPPLIER_PLACE1_END_CONNECTORS = auto()
    ELEMENTS_CLIENT_PLACE2_END_CONNECTORS = auto()
    ELEMENTS_CONTAINED_EA_DIAGRAMS = auto()
    ELEMENTS_CONTAINED_EA_CLASSIFIERS = auto()
    ELEMENTS_CLASSIFIER = auto()
    EXPLICIT_OBJECTS_EA_OBJECT_NAME = auto()
    EXPLICIT_OBJECTS_EA_OBJECT_NOTES = auto()
    EXPLICIT_OBJECTS_EA_GUID = auto()
    PACKAGEABLE_OBJECTS_PARENT_EA_ELEMENT = auto()
    PACKAGES_CONTAINED_EA_PACKAGES = auto()
    PACKAGES_VIEW_TYPE = auto()
    REPOSITORIED_OBJECTS_EA_REPOSITORY = auto()
    STEREOTYPE_EA_STEREOTYPE_GROUP = auto()
    STEREOTYPEABLE_OBJECTS_EA_OBJECT_STEREOTYPES = auto()
    CONNECTORS_DIRECTION_TYPE_NAME = auto()
    CONNECTORS_ELEMENT_TYPE_NAME = auto()
    CONNECTORS_SOURCE_CARDINALITY = auto()
    CONNECTORS_DEST_CARDINALITY = auto()
    ELEMENT_COMPONENTS_CONTAINING_EA_CLASSIFIER = auto()
    ELEMENT_COMPONENTS_CLASSIFYING_EA_CLASSIFIER = auto()
    ELEMENT_COMPONENTS_UML_VISIBILITY_KIND = auto()
