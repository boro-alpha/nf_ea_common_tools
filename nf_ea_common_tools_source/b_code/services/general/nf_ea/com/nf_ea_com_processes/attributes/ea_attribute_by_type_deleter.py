...
def delete_nf_ea_attribute_instances_for_type(
    ...
    ea_attribute_instances_dataframe = \
        __get_ea_attribute_instances_dataframe(
            ea_tools_session_manager=ea_tools_session_manager,
            ea_repository=ea_repository,
            attribute_type_ea_guid=attribute_type_ea_guid)

    for ea_attribute_instances_tuple in ea_attribute_instances_dataframe.itertuples():
...
def __get_ea_attribute_instances_dataframe(
        ...
        attribute_type_ea_guid: str):
    ea_attributes = \
        ea_tools_session_manager.nf_ea_com_endpoint_manager.nf_ea_com_universe_manager.get_ea_attributes(
            ea_repository=ea_repository)
    ...
    ea_attribute_instances_dataframe = \
        ea_attributes.loc[
            ea_attributes[classifying_classifier_column_name] == attribute_type_nf_uuid]

    log_message(
        'Attribute type: ' + attribute_type_ea_guid + ' has ' + str(
            ea_attribute_instances_dataframe.shape[0]) + ' instances')
