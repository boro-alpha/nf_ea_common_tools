...
from ea_interop_service_source.b_code.i_dual_objects.collections.i_dual_attribute_collection import IDualAttributeCollection
...
def remove_nf_ea_attribute(
    ...
    with \
            i_dual_repository.get_attribute_by_guid(
...
def __remove_nf_ea_attribute(
        ...
        ea_attribute_guid: str):
    if not isinstance(i_dual_attribute, IDualAttribute):
        log_message(
            ea_attribute_guid + " Warning: Attribute not found")

        return

    parent_i_dual_element = \
        __get_parent_i_dual_element(
            i_dual_repository=i_dual_repository,
            i_dual_attribute=i_dual_attribute)

    if not isinstance(parent_i_dual_element, IDualElement):
        log_message(
            ea_attribute_guid + " Warning: Parent Element not found")
...
def __get_index_to_delete(
        ...
        ea_attribute_guid: str) \
        -> int:
    for index in range(i_dual_attribute_collection.count):
        attribute_at_index = \
            i_dual_attribute_collection.get_at(
                index)

        if attribute_at_index.attribute_guid == ea_attribute_guid:
            return \
                index
...
def __report_attribute(
    ...
    log_message(
        attribute_guid + ' : Attribute \'' + attribute_name + '\' to be removed')


def __report_deletion_success(
        ea_attribute_guid: str):
    log_message(
        ea_attribute_guid + " : Attribute removed")


def __report_deletion_failure(
        ea_attribute_guid: str):
    log_message(
        ea_attribute_guid + " : Attribute removal failed.  Failed to find attribute in parent collection")
