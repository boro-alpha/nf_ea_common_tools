...
def __map_matched_ea_packages(
    ...
    log_message(
        'Mapping ' + str(matched_ea_packages.shape[0]) + ' packages')

    i_dual_repository = \
        EaRepositoryMappers.get_i_dual_repository(
            ea_repository=ea_repository)

    for index, ea_package_row in matched_ea_packages.iterrows():
...
def __map_matched_ea_package(
    ...
    nf_uuid = \
        ea_package_row[nf_uuid_column_name]

    ea_guid = \
        ea_package_row[ea_guids_column_name]

    ea_package = \
        i_dual_repository.get_package_by_guid(
            package_ea_guid=ea_guid)

    if not isinstance(ea_package, IDualPackage):
        raise \
            TypeError
...
def __load_unmatched_ea_packages(
    ...
    log_message(
        'Loading ' + str(unmatched_ea_packages.shape[0]) + ' packages')
