...
def load_ea_xml_file_to_ea_repository(
    ...
    log_message(
        message='Loading ' + load_xml_file_full_path + ' to EA repository')
    ...
    if not isinstance(project_interface, IDualProject):
        raise \
            TypeError
...
def __get_load_package_ea_guid(
        ...
        -> str:
    top_level_model = \
        i_dual_repository.models.get_at(0)
    ...
    if model_packages.count == 0:
        load_package_ea_guid = \
            __create_load_package_and_get_its_ea_guid(
                model_packages=model_packages)
    else:
        top_level_package = \
            top_level_model.packages.get_at(0)
