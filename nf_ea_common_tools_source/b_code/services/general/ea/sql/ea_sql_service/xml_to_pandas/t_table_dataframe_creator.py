from nf_ea_common_tools_source.b_code.services.general.ea.sql.ea_sql_service.xml_to_pandas.node_value_getter import get_node_value
...
def create_t_table_dataframe(
        ...
        -> pandas.DataFrame:
    log_message(
        message='creating dataframe using column type: ' + str(column_types_enum))

    t_table_dataframe_column_nf_names_dictionary = \
        __get_t_table_dataframe_nf_column_names_dictionary(
            column_types_enum=column_types_enum)

    t_table_dataframe_column_ea_names_dictionary = \
        __get_t_table_dataframe_column_names_dictionary(
            column_types_enum=column_types_enum)

    t_table_dataframe = \
        __create_t_table_dataframe(
    ...
    log_message(
        message='created dataframe using column type: ' + str(column_types_enum))
...
def __get_t_table_dataframe_nf_column_names_dictionary(
    ...
    for column_type in column_types_enum:
        column_name = \
            column_type.nf_column_name

        t_table_dataframe_column_names_dictionary[column_type] = \
            column_name
...
def __create_t_table_dataframe(
    ...
    for dataset_node in xml_element_tree.getroot():
        t_table_dictionary = \
            __process_dataset_node(
...
def __process_row_node(
    ...
    for column_type, ea_name in ea_names_dictionary.items():
        cell_value_dictionary[column_type] = \
            row_node.find(
                ea_name)
    ...
    for column_type, nf_name in nf_names_dictionary.items():
        t_row_dictionary[nf_name] = \
            get_node_value(
                cell_value_dictionary[column_type])

    table_dictionary[current_row_count] = \
        t_row_dictionary
