...
from nf_ea_common_tools_source.b_code.services.general.ea.xml.nf_ea_xml_adders.nf_ea_xml_add_helpers import add_xml_row_element_to_xml_table_element
...
def add_ea_stereotype_usages_to_xml_root_element(
        ...
        xml_root: Element) \
        -> Element:
    xml_element_for_stereotype_usages = \
        SubElement(
            xml_root,
            'Table')
    ...
    if stereotype_property_type_column_name not in stereotype_usage_with_names_copy.columns:
        stereotype_usage_with_names_copy[stereotype_property_type_column_name] = str()

    for index, stereotype_usage_row in stereotype_usage_with_names_copy.iterrows():
        __add_ea_stereotype_usage_to_xml_tree(
            client_nf_uuid=stereotype_usage_row[client_nf_uuid_column_name],
            stereotype_nf_uuid=stereotype_usage_row[stereotype_nf_uuid_column_name],
            stereotype_name=stereotype_usage_row[stereotype_name_column_name],
            stereotype_property_type=stereotype_usage_row[stereotype_property_type_column_name],
...
def __add_ea_stereotype_usage_to_xml_tree(
    ...
    if not isinstance(stereotype_nf_uuid, str):
        log_message(
            message='Cannot find stereotype of stereotype_usage for client: ' + client_nf_uuid)
