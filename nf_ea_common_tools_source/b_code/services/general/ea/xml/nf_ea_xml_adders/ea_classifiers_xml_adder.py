...
from nf_ea_common_tools_source.b_code.services.general.ea.xml.common_knowledge.maps.ea_guids_to_ea_identifiers_for_connectors_mappings import \
...
from nf_ea_common_tools_source.b_code.services.general.ea.xml.common_knowledge.maps.ea_guids_to_ea_identifiers_for_packages_mappings import \
    EaGuidsToEaIdentifiersForPackagesMappings
from nf_ea_common_tools_source.b_code.services.general.ea.xml.nf_ea_xml_adders.nf_ea_xml_add_helpers import \
...
def add_ea_classifiers_to_xml_root_element(
    ...
    non_classified_ea_classifiers = \
        ea_classifiers.loc[ea_classifiers[classifier_column_name] == DEFAULT_NULL_VALUE]

    next_ea_identifier = \
        start_ea_identifier + 1

    for ea_classifier_tuple in non_classified_ea_classifiers.itertuples():
    ...
    for ea_classifier_tuple in classified_ea_classifiers.itertuples():
        __add_ea_classifier_to_xml_tree(
            ea_classifier_tuple=ea_classifier_tuple,
            ea_classifier_identifier=next_ea_identifier,
            xml_element_for_classifiers=xml_element_for_classifiers,
...
def __add_ea_classifier_to_xml_tree(
    ...
    if proxy_load:
        if ea_classifier_type != EaElementTypes.PROXY_CONNECTOR.type_name:
            return
    ...
    containing_ea_package_ea_guid = \
        NfUuidsToEaGuidsMappings.get_ea_guid(
            containing_ea_package_nf_uuid)

    ea_package_parent_ea_identifier = \
        EaGuidsToEaIdentifiersForPackagesMappings.map.get_ea_identifier(
            containing_ea_package_ea_guid)
    ...
    if ea_classifier_note == DEFAULT_NULL_VALUE:
        ea_classifier_note = ''
    ...
    if ea_classifier_classifier_nf_uuid == DEFAULT_NULL_VALUE:
        ea_classifier_classifier_ea_guid = ''

        ea_classifier_classifier_id = \
            0
        ...
        if ea_classifier_classifier_ea_guid in EaGuidsToEaIdentifiersForObjectsMappings.map.get_map():
            ea_classifier_classifier_id = \
                EaGuidsToEaIdentifiersForObjectsMappings.map.get_ea_identifier(
                    ea_classifier_classifier_ea_guid)

        elif ea_classifier_classifier_ea_guid in EaGuidsToEaIdentifiersForConnectorsMappings.map.get_map():
            ea_classifier_classifier_id = \
                EaGuidsToEaIdentifiersForConnectorsMappings.map.get_ea_identifier(
                    ea_classifier_classifier_ea_guid)

        else:
            log_message(
                'Cannot find identifier for classifier\'s classifier with guid ' + str(ea_classifier_classifier_ea_guid))
    ...
    stereotype_names = \
        stereotype_usage_with_names[stereotype_usage_with_names[NfEaComColumnTypes.STEREOTYPE_CLIENT_NF_UUIDS.column_name] == ea_classifier_nf_uuid][
            NfEaComColumnTypes.STEREOTYPE_NAMES.column_name].values

    if len(stereotype_names) == 0:
        stereotype_name = ''

    else:
        stereotype_name = stereotype_names[0]
