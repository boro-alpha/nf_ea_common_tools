...
import sys
from nf_ea_common_tools_source.b_code.services.general.ea.xml.nf_ea_xml_adders.ea_package_xml_adder import add_ea_package_to_xml_tree
...
def add_ea_packages_to_xml_root_element(
        ...
        xml_element_for_packages_as_objects=None) -> tuple:
    nf_uuid_column_name = \
    ...
    if xml_element_for_packages_as_objects is None:
        xml_element_for_packages_as_objects = \
            __create_xml_element_for_packages_as_objects(
                xml_root=xml_root)
    ...
    for index, ea_package_row in ea_packages.iterrows():
    ...
    start_ea_identifier_for_new_objects += index + 1

    if len(recyclable_packages) == len(ea_packages):
        log_message(
