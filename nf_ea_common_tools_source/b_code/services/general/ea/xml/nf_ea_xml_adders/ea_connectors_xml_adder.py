...
from nf_ea_common_tools_source.b_code.services.general.ea.xml.common_knowledge.maps.connector_types_to_destination_is_aggregate_mappings import \
    get_destination_is_aggregate_from_connector_type
from nf_ea_common_tools_source.b_code.services.general.ea.xml.common_knowledge.maps.connector_types_to_direction_strings_mappings import \
...
def add_ea_connectors_to_xml_root_element(
        ...
        -> tuple:
    if xml_element_for_connectors is None:
        xml_element_for_connectors = \
            __create_xml_element_for_connectors(
                xml_root=xml_root)

    next_ea_identifier = \
        start_ea_identifier + 1

    for ea_connector_tuple in ea_connectors.itertuples():
...
def __add_ea_connector_to_xml_tree(
    ...
    if ea_connector_name == DEFAULT_NULL_VALUE:
        ea_connector_name = ''

    if ea_connector_notes == DEFAULT_NULL_VALUE:
        ea_connector_notes = ''

    stereotype_names = \
        stereotype_usage_with_names[stereotype_usage_with_names[NfEaComColumnTypes.STEREOTYPE_CLIENT_NF_UUIDS.column_name] == ea_connector_nf_uuid][
    ...
    if ea_connector_nf_uuid != DEFAULT_NULL_VALUE:
